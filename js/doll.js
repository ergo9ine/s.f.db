...
function Skill(y,x){
	...
	src==23?Sdesc=
		y==79||y==185?`야간작전에서 ${time[1]}초 동안 자신의 화력을 ${x.Fx.dmg[1]}%(주간 보정치 : ${dmg[1]}%) 상승시키며 사격 중 장탄수를 ${x.Fx.ammo[1]}발 추가한다.`:
		y==80||y==81||y==110||y==121?`자신의 명중을 ${hit[1]}% 상승시키며 지속시간 동안 모든 공격이 치명타로 적용된다.`:
		y==82||y==88||y==89||y==125||y==195?`${time[1]}초 동안 자신의 화력을 ${dmg[1]}% 상승시키며 사격 중 장탄수를 ${x.Fx.ammo[1]}발 추가한다.`:
		`패시브 스킬 <br> 매 3회 공격 시 그 다음 공격은 타깃에게 ${dmg[1]/10}배의 피해를 입힌다.`:
